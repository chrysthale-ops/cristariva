...
function cr363Title(card){return card?((card.oracle==='amour'||card.oracle==='tarot')?(card.name||''):(CR363_SHORT_TITLES[Number(card.id)]||card.name||'')):'';}
function cr363ApplyTitles(){
  try{
    ...
    if(typeof state==='object'&&state){
      if(Array.isArray(state.draw))state.draw.forEach(card=>{if(card?.oracle==='amour'||card?.oracle==='tarot')return;const title=CR363_SHORT_TITLES[Number(card.id)];if(title)card.name=title;});
      ['relation','date'].forEach(key=>{const card=state[key];if(card?.oracle==='amour'||card?.oracle==='tarot')return;const title=CR363_SHORT_TITLES[Number(card?.id)];if(card&&title)card.name=title;});
    }
  }catch(e){}
}
cr363ApplyTitles();
